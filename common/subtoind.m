...
if length(siz) ~= numargin-1
    %Adjust input
    if length(siz)<numargin-1
        %Adjust for trailing singleton dimensions
        siz = [siz ones(1,numargin-length(siz)-1)];
    else
        %Adjust for linear indexing on last element
        siz = [siz(1:numargin-2) prod(siz(numargin-1:end))];
    end
end
